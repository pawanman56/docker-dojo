...
router.get('/', function(req, res, next) {
  ...
  async.parallel([
      function(callback){
        ...
        rest.get('http://localhost:8080/api/artists/search?artist=' + artist).on('complete', function(data) {
          console.log('Artist ID is ' + data["id"]);
          rest.get('http://localhost:8080/api/charts/' + data["id"]).on('complete', function(data) {
      ...
      function(callback){
          ...
          rest.get('http://localhost:8080/api/tracks/search?title=' + title + '&artist=' + artist).on('complete', function(data) {
            console.log('Title ID is ' + data["id"]);

            rest.get('http://localhost:8080/api/covers/' + data["id"]).on('complete', function(data) {
